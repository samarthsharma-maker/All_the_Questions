...
function create_bloat_files() {
    ...
    touch ${BASE_DIR}/data/train.csv
    touch ${BASE_DIR}/data/test.csv  
    touch ${BASE_DIR}/data/validation.csv
    # Create one actual 80MB file
    dd if=/dev/zero of=${BASE_DIR}/data/large_dataset.csv bs=1M count=80 2>/dev/null
    
    # Models directory (~40MB)
    mkdir -p ${BASE_DIR}/models
    dd if=/dev/zero of=${BASE_DIR}/models/model_v1.pkl bs=1M count=20 2>/dev/null
    dd if=/dev/zero of=${BASE_DIR}/models/model_v2.pkl bs=1M count=20 2>/dev/null
    
    # __pycache__ (~20MB)
    mkdir -p ${BASE_DIR}/__pycache__
    dd if=/dev/zero of=${BASE_DIR}/__pycache__/app.cpython-39.pyc bs=1M count=10 2>/dev/null
    dd if=/dev/zero of=${BASE_DIR}/__pycache__/config.cpython-39.pyc bs=1M count=10 2>/dev/null
    
    # .pytest_cache (~30MB)
    mkdir -p ${BASE_DIR}/.pytest_cache
    dd if=/dev/zero of=${BASE_DIR}/.pytest_cache/cache.db bs=1M count=30 2>/dev/null
    
    # Tests directory (small files)
    mkdir -p ${BASE_DIR}/tests
    cat > ${BASE_DIR}/tests/test_app.py <<'EOF'
def test_health():
    assert True
...
def test_config():
    assert True
EOF
    cat > ${BASE_DIR}/tests/test_utils.py <<'EOF'
def test_validation():
    assert True
EOF
    ...
    dd if=/dev/zero of=${BASE_DIR}/.git/objects/pack.idx bs=1M count=20 2>/dev/null

    # venv (~30MB)
    mkdir -p ${BASE_DIR}/venv/lib/python3.9/site-packages
    dd if=/dev/zero of=${BASE_DIR}/venv/lib/python3.9/site-packages/large.so bs=1M count=30 2>/dev/null
    
    # Logs (~10MB)
    mkdir -p ${BASE_DIR}/logs
    dd if=/dev/zero of=${BASE_DIR}/logs/app.log bs=1M count=10 2>/dev/null
